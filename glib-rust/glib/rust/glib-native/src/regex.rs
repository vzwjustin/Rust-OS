...
impl<'a> Parser<'a> {
    ...
    fn parse_atom(&mut self) -> Result<Node, RegexError> {
        ...
        match c {
            ...
            '[' => self.parse_class(),
            '\\' => self.parse_escape(),
            _ => Ok(Node::Char(if self.caseless {
    ...
    fn parse_class(&mut self) -> Result<Node, RegexError> {
        ...
        while let Some(c) = self.peek() {
            ...
            let item = if c == '\\' {
                ...
                ClassItem::Single(if self.caseless {
                    c.to_ascii_lowercase()
                } else {
                    c
                })
    ...
    fn parse_class_escape(&mut self) -> Result<ClassItem, RegexError> {
        ...
        match c {
            ...
            'D' => Ok(ClassItem::NonDigit),
            'w' => Ok(ClassItem::Word),
            'W' => Ok(ClassItem::NonWord),
            's' => Ok(ClassItem::Space),
            'S' => Ok(ClassItem::NonSpace),
            ...
            ']' => Ok(ClassItem::Single(']')),
            '-' => Ok(ClassItem::Single('-')),
            _ => Ok(ClassItem::Single(if self.caseless {
                c.to_ascii_lowercase()
            } else {
                c
            })),
            ...
            _ => {
                if c.is_ascii_digit() {
                    ...
                    if num > self.capture_count as u32 {
                        ...
                        if (num as i32) > self.max_backref {
                            self.max_backref = num as i32;
                        }
                        Ok(Node::Sequence(Vec::new()))
                    }
                } else {
                    Ok(Node::Char(if self.caseless {
                        c.to_ascii_lowercase()
                    } else {
                        c
                    }))
    ...
    fn parse_brace_quantifier(&mut self) -> Result<(u32, Option<u32>), RegexError> {
        ...
        let max = if self.peek() == Some(',') {
            ...
            if max_str.is_empty() {
                ...
                Some(max_str.parse().map_err(|_| RegexError::QuantifierTooBig)?)
            }
        } else {
            Some(min)
        };
        match self.next() {
            Some('}') => {}
            _ => return Err(RegexError::QuantifiersOutOfOrder),
        }
        Ok((min, max))
    }
}
...
impl<'a> Matcher<'a> {
    fn new(input: &'a [char], caseless: bool, num_captures: usize) -> Self {
        Self {
            input,
            caseless,
            captures: vec![None; num_captures + 1],
            num_captures,
        }
    }
    ...
    fn try_match(
        ...
        match node {
            Node::Char(ch) => {
                if pos < self.input.len() && self.char_eq(*ch, self.input[pos]) {
                    vec![pos + 1]
                } else {
                    vec![]
                }
            ...
            Node::Class { negated, items } => {
                if pos < self.input.len() && self.match_class(*negated, items, self.input[pos]) {
                    vec![pos + 1]
                } else {
                    vec![]
                }
            ...
            Node::Quantifier {
                ...
                max,
                greedy,
            } => self.match_quant_rec(child, *min, *max, *greedy, pos, 0, captures),
        }
    }
    ...
    fn match_quant_rec(
        ...
        if can_more {
            for end in self.try_match(child, pos, captures) {
                ...
                let mut caps_copy = captures.clone();
                let rest =
                    self.match_quant_rec(child, min, max, greedy, end, count + 1, &mut caps_copy);
                if !rest.is_empty() {
                    *captures = caps_copy;
                    rec_ends.extend(rest);
                }
            }
        }
        if greedy {
...
impl Regex {
    pub fn new(
        ...
        Ok(Self {
            ...
            max_backref: parser.max_backref,
    ...
    pub fn get_max_backref(&self) -> i32 {
        self.max_backref
    }
    pub fn get_capture_count(&self) -> i32 {
        self.capture_count
    }
    pub fn get_has_cr_or_lf(&self) -> bool {
        self.has_cr_or_lf
    }
    pub fn get_max_lookbehind(&self) -> i32 {
        0
    }
    pub fn get_compile_flags(&self) -> RegexCompileFlags {
        self.compile_flags
    }
    pub fn get_match_flags(&self) -> RegexMatchFlags {
        self.match_flags
    }
    pub fn get_string_number(&self, _name: &str) -> i32 {
        -1
    }
    ...
    pub fn r#match(&self, string: &str, _match_flags: RegexMatchFlags) -> MatchInfo {
        ...
        match matcher.find(&self.ast, 0) {
            Some((start, end)) => MatchInfo {
                regex_pattern: self.pattern.clone(),
                string: string.to_owned(),
                matched: true,
                match_count: self.capture_count + 1,
                start_pos: start,
                end_pos: end,
                is_partial: false,
            ...
            None => MatchInfo {
                ...
                end_pos: 0,
                is_partial: false,
                captures: vec![None; self.capture_count as usize + 1],
                input_chars: input,
            },
        }
    }
    ...
    pub fn match_full(
        ...
        string_len: i64,
        start_position: i32,
        _match_flags: RegexMatchFlags,
    ) -> Result<MatchInfo, RegexError> {
        let s = if string_len >= 0 {
            &string[..(string_len as usize).min(string.len())]
        } else {
            string
        };
    ...
    pub fn split(&self, string: &str, _match_flags: RegexMatchFlags) -> Vec<String> {
        ...
        while pos < input.len() {
            if let Some((start, end)) = matcher.find(&self.ast, pos) {
                if start == end {
                    result.push(input[pos..start].iter().collect());
                    pos = start + 1;
                } else {
                    result.push(input[pos..start].iter().collect());
                    pos = end;
                        ...
                        if let Some(&n) = repl.peek() {
                            if n.is_ascii_digit() {
                                ...
                                if idx < matcher.captures.len() {
                                    if let Some((cs, ce)) = matcher.captures[idx] {
                                        result.extend(input[cs..ce].iter());
                                    }
                                }
                                continue;
                            }
    ...
    pub fn escape_string(string: &str, length: i32) -> String {
        let s = if length < 0 {
            ...
            &string[..(length as usize).min(string.len())]
        };
        let mut result = String::new();
        for c in s.chars() {
            match c {
...
impl MatchInfo {
    ...
    pub fn fetch_all(&self) -> Vec<String> {
        ...
        for cap in &self.captures {
            if let Some((s, e)) = cap {
                result.push(self.input_chars[*s..*e].iter().collect());
            }
        }
        result
    }
    ...
    pub fn next(&mut self) -> bool {
        ...
        let start = if self.end_pos == self.start_pos {
            self.end_pos + 1
        } else {
            self.end_pos
        };
        if start > self.input_chars.len() {
            self.matched = false;
            return false;
        }
        ...
        match matcher.find(&re.ast, start) {
            Some((s, e)) => {
                self.start_pos = s;
                self.end_pos = e;
                self.captures = matcher.captures;
                true
            }
            None => {
                self.matched = false;
...
mod tests {
    ...
    fn literal_no_match() {
        ...
        assert!(!re
            .r#match("hello world", RegexMatchFlags::DEFAULT)
            .matches());
    }
    ...
    fn anchors() {
        ...
        assert!(re2
            ...
            .matches());
        assert!(!re2
            .r#match("world hello", RegexMatchFlags::DEFAULT)
            .matches());
    }
    ...
    fn capture_backtrack() {
        ...
        assert!(info.matches());
        assert_eq!(info.fetch(1), Some("li".to_owned()));
    }
    ...
    fn word_boundary() {
        ...
        .unwrap();
        assert!(re
            .r#match("a word here", RegexMatchFlags::DEFAULT)
            .matches());
