...
pub const MEM_ALIGN: Size = core::mem::size_of::<usize>();
...
fn validate_alignment(alignment: Size) {
    ...
    if !alignment.is_multiple_of(core::mem::size_of::<usize>()) {
        panic!(
...
mod tests {
    ...
    fn aligned_alloc_overflow_panics() {
        // n_blocks * n_block_bytes must overflow Size so the check fires
        // deterministically (a huge-but-valid size could otherwise be satisfied
        // by an overcommitting allocator instead of panicking).
        let a = Size::MAX / 10 + 10;
        let result = std::panic::catch_unwind(|| {
            let _ = aligned_alloc(a, 16, 16);
        });
        assert!(result.is_err());
    }
