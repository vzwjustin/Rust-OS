...
impl GString {
    ...
    pub fn new_len(init: Option<&[u8]>, len: isize) -> Self {
        if len < 0 {
            return Self::new(init.and_then(|b| core::str::from_utf8(b).ok()));
        }
    ...
    pub fn as_str(&self) -> &str {
        core::str::from_utf8(&self.buf[..self.len]).expect("GString UTF-8")
    }
    ...
    fn insert_len(&mut self, pos: isize, val: &[u8], len: Size) -> &mut Self {
        ...
        if val_start >= buf_start && val_start <= buf_end {
            ...
            if len > precount {
                ...
                unsafe {
                    core::ptr::copy(src as *const u8, self.buf.as_mut_ptr().add(dst), count);
                }
            }
        } else {
            self.buf.resize(self.len + len + 1, 0);
