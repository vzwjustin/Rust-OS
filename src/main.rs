...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    #[cfg(not(test))]
    {
        ...
        //
        // ACTIVE DEFAULT DESKTOP RENDERER: `desktop::render_desktop()`
        // (`src/desktop/window_manager.rs`) is the primary, GNOME-Shell-style
        // kernel-rendered shell for a normal graphical boot. Whenever the
        // 32-bit framebuffer desktop is available, `modern_desktop_main_loop`
        // is entered regardless of whether userspace init (GNOME session
        // bootstrap) also spawned — that loop renders the full window_manager
        // shell (panel, dock, overview, app grid, quick settings) *and* still
        // forwards input to and composites `wayland::render_clients()` on top
        // of it every frame, so any real Wayland client surfaces continue to
        // display. `mutter.rs`'s bare synthetic client (flat top bar + dock +
        // 5 icon squares) is no longer launched from this loop (see comment
        // in `modern_desktop_main_loop`) since window_manager.rs supersedes
        // it visually. `userspace_session_loop` (no kernel desktop chrome at
        // all) is now only a fallback for the rare case where the kernel
        // desktop/window manager failed to initialize but userspace init
        // still spawned.
        ...
        } else if userspace_spawned {
            // Kernel window manager desktop isn't available (framebuffer not
            // 32bpp, or window_manager init failed) but userspace init still
            // spawned. Fall back to a minimal compositor/idle loop that
            // services the userspace process, forwards input to Wayland
            // clients, and renders their surfaces without a kernel desktop.
            crate::serial_println!(
                "Boot: window_manager desktop unavailable — entering userspace session loop"
            );
